...
For general Temporal AI/LLM patterns (retries, rate limits, timeouts, multi-agent orchestration) see [references/core/ai-patterns.md](../../core/ai-patterns.md). For Spring Boot autoconfigure mechanics (worker lifecycle, `@WorkflowImpl`, `@ActivityImpl`, auto-discovery) see [references/java/integrations/spring-boot.md](spring-boot.md).
...
Determinism still applies: plain tools execute on the workflow thread, so they must follow the same rules as workflow code (no I/O, no system clock, no random sources). See [references/java/determinism.md](../determinism.md) and [references/core/determinism.md](../../core/determinism.md).
...
- [references/java/integrations/spring-boot.md](spring-boot.md) — required companion module; covers `WorkflowClient` injection, worker lifecycle, auto-discovery, testing.
- [references/core/ai-patterns.md](../../core/ai-patterns.md) — language-agnostic AI/LLM patterns (Activities wrap LLM calls, retry centralization, multi-agent orchestration).
- [references/java/determinism.md](../determinism.md) and [references/core/determinism.md](../../core/determinism.md) — replay rules that plain tools and `@SideEffectTool` tools must respect.
