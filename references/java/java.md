...
- **`references/java/standalone-activities.md`** - Run an Activity directly from a Client (no Workflow) using `ActivityClient.execute` / `start` / `listExecutions` / `countExecutions` (Pre-release)
