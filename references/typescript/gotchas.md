...
For detailed guidance on error classification and retry policies, see [error-handling.md](error-handling.md).
...
`setTimeout` works in workflows (the SDK mocks it), but `sleep()` from `@temporalio/workflow` is preferred because its interaction with cancellation scopes is more intuitive. See Timers in [references/typescript/patterns.md](patterns.md).
