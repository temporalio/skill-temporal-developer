...
| LangGraph (`temporalio.contrib.langgraph`, Pre-release) | Python | Runs LangGraph Graph-API nodes and Functional-API tasks as Temporal Activities or on the Workflow thread; per-node/task `execute_in` plus Temporal activity options | `references/python/integrations/langgraph.md` | `references/python/ai-patterns.md`, `references/core/ai-patterns.md` |
