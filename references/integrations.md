...
| OpenAI Agents SDK — SandboxAgent (`temporalio.contrib.openai_agents`) | Python | Routes `SandboxAgent` session operations (exec, read, write, PTY) through Temporal activities via `SandboxClientProvider` + `temporal_sandbox_client(name)`; supports Daytona and Unix-local backends | `references/python/integrations/openai-sandbox.md` | `references/python/ai-patterns.md`, `references/core/ai-patterns.md` |
