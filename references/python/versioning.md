...
For conceptual overview and guidance on choosing an approach, see [references/core/versioning.md](../core/versioning.md).
...
For long-running Pinned Workflows that use Continue-as-New, detect a new Target Worker Deployment Version on `workflow.info()` and continue-as-new with `ContinueAsNewVersioningBehavior.AUTO_UPGRADE` so the new run starts on the Target Version. See [references/core/versioning.md](../core/versioning.md) for the conceptual model.
