...
The local Temporal documentation only shows a Go example for this Public-Preview feature. The Python SDK exposes equivalent capabilities, but the specific token names are not described in the docs clone — confirm them in the upstream SDK before writing production code.
...
Python-side concrete names (not in the local docs):

- The Workflow-Info accessor for the flag: <!-- VERIFY: exact attribute or method name on temporalio.workflow.Info for the target-version-changed flag; check upstream temporalio/sdk-python release notes (Public Preview) -->
- The Continue-as-New API that accepts initial versioning behavior: <!-- VERIFY: which `temporalio.workflow.continue_as_new` parameter / variant accepts an initial versioning behavior, and the exact name of the AUTO_UPGRADE option, in upstream temporalio/sdk-python -->

Until these are verified against the upstream SDK, write the *check* in terms of the documented concept, not a specific Python token:

```python
# Pseudocode — token names are SDK-version-specific and not in the docs clone.
# Verify against upstream temporalio/sdk-python before using in production.
@workflow.defn(versioning_behavior=VersioningBehavior.PINNED)
class LongRunningEntity:
    @workflow.run
    async def run(self, state: State) -> None:
        while not state.done:
            await self._do_unit_of_work(state)

            # Check after each meaningful Workflow Task — the flag refreshes
            # only across Workflow Task boundaries.
            if workflow.info().<!-- VERIFY: attribute name -->:
                workflow.continue_as_new(
                    state,
                    # Pass an initial versioning behavior so the new run lands
                    # on its Worker Deployment's Target Version.
                    # <!-- VERIFY: parameter name and AUTO_UPGRADE constant name -->
                )
```
